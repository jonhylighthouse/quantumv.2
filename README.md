...
```
ID | Nombre | SponsorID | Puntos | MetaPersonal | Activo | FechaActualizacion | TiempoDisponible | Intereses | Fortalezas | ObjetivosPersonales | Observaciones | ProximosPasos
```

3. Agrega los datos iniciales desde la fila 2:

| ID | Nombre | SponsorID | Puntos | MetaPersonal | Activo | FechaActualizacion | TiempoDisponible | Intereses | Fortalezas | ObjetivosPersonales | Observaciones | PróximosPasos |
|----|--------|-----------|--------|--------------|--------|-------------------|------------------|-----------|------------|---------------------|---------------|---------------|
| 1  | Joy    |           | 201    | 300          | TRUE   | 2026-05-31        | Tiempo completo  | Estrategia, líderes | Constructor | Empoderar red al 21%| Gran líder   | Reunión Jon / Wall |
| 2  | Jon    | 1         | 85     | 150          | TRUE   | 2026-05-31        | 12 horas/semana  | Liderazgo, equipo  | Educador   | Ayudar a Mamá       | Comprometido  | Academia con Mamá  |
| 3  | Wall   | 1         | 25     | 150          | TRUE   | 2026-05-31        | 8 horas/semana   | Venta, relaciones  | Comercial  | Llegar al 6%        | Muy carismático| Talleres de venta  |
| 4  | Mamá   | 2         | 25     | 150          | TRUE   | 2026-05-31        | 6 horas/semana   | Bienestar familiar | Técnico    | Consumo 150 PP      | Ritmo tranquilo| Selección productos|

> **Nota:** `SponsorID` vacío = raíz del árbol (Joy). El campo apunta al `ID` del sponsor (upline). Todos estos campos permiten una gestión empática y personalizada de tu red.
...
3. Configura:
   ...
   - **Quién tiene acceso:** `Cualquier usuario` (Muy importante para permitir la conexión desde la app web).
4. Haz clic en **Implementar**.
5. Autoriza los permisos que solicite Google.
6. **Copia la URL** oficial de la implementación que se te presenta en pantalla.
   - ⚠️ **IMPORTANTE:** La URL correcta debe verse como:  
     `https://script.google.com/macros/s/AKfycb.../exec`  
   - ❌ **NO COPIES** la dirección desde la barra del navegador tras probar el enlace. Google te redirigirá automáticamente a una dirección temporal que empieza con `https://script.googleusercontent.com/macros/echo?...`. Esa URL contiene una clave de sesión de corta duración y causará un error `Failed to fetch / Sin conexión` al ingresarla en Quantum.
